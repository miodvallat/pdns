...
SMySQL::SMySQL(std::shared_ptr<Logr::Logger> log, string database, string host, uint16_t port, string msocket, string user,
  ...
  d_database(std::move(database)), d_host(std::move(host)), d_msocket(std::move(msocket)), d_user(std::move(user)), d_password(std::move(password)), d_group(std::move(group)), d_timeout(timeout), d_port(port), d_setIsolation(setIsolation), d_threadCleanup(threadCleanup), d_slog(log)
{
  connect();
}
