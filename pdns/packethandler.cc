...
bool PacketHandler::addCDNSKEY(DNSPacket& p, std::unique_ptr<DNSPacket>& r, SOAData &sd) // NOLINT(readability-identifier-length)
{
  ...
  if(::arg().mustDo("direct-dnskey")) {
    B.lookup(QType(QType::CDNSKEY), sd.zonename, sd.domain_id, &p);
...
bool PacketHandler::addCDS(DNSPacket& p, std::unique_ptr<DNSPacket>& r, SOAData &sd) // NOLINT(readability-identifier-length)
{
  ...
  for (const auto& value : d_dk.getEntryPoints(sd.zonename)) {
    ...
    for(auto const &digestAlgo : digestAlgos){
      rr.dr.setContent(std::make_shared<DSRecordContent>(makeDSFromDNSKey(sd.zonename, value.first.getDNSKEY(), pdns::checked_stoi<uint8_t>(digestAlgo))));
      r->addRecord(DNSZoneRecord(rr));
      haveOne=true;
    }
  }
  ...
  if(::arg().mustDo("direct-dnskey")) {
    B.lookup(QType(QType::CDS), sd.zonename, sd.domain_id, &p);
...
vector<DNSZoneRecord> PacketHandler::getBestReferralNS(DNSPacket& p, const DNSName &target)
{
  ...
  do {
    if(subdomain == d_sd.zonename) { // stop at SOA
      break;
    }
...
void PacketHandler::getBestDNAMESynth(DNSPacket& p, DNSName &target, vector<DNSZoneRecord> &ret)
{
  ...
  DNSName subdomain(target);
  do {
    DLOG(g_log<<"Attempting DNAME lookup for "<<subdomain<<", d_sd.zonename="<<d_sd.zonename<<endl);
...
bool PacketHandler::getBestWildcard(DNSPacket& p, const DNSName &target, DNSName &wildcard, vector<DNSZoneRecord>* ret)
{
  ...
  while( subdomain.chopOff() && !haveSomething )  {
    ...
    while(B.get(rr)) {
      ...
      if (rr.dr.d_type == QType::LUA && !isPresigned()) {
        ...
        if(rec->d_type == QType::CNAME || rec->d_type == p.qtype.getCode() || (p.qtype.getCode() == QType::ANY && rec->d_type != QType::RRSIG)) {
          ...
          try {
            auto recvec=luaSynth(rec->getCode(), target, rr, d_sd.zonename, p, rec->d_type, s_LUA);
    ...
    if ( subdomain == d_sd.zonename || haveSomething ) { // stop at SOA or result
      break;
    }
...
void PacketHandler::doAdditionalProcessing(DNSPacket& p, std::unique_ptr<DNSPacket>& r)
{
  ...
  for(auto& rr : rrs) {
    if(rr.dr.d_place != DNSResourceRecord::ADDITIONAL) {
        ...
        case QType::NAPTR: {
          ...
          if (flags.find('a') != string::npos) {
            ...
            while(B.get(dzr)) {
              ...
              if(content.isPartOf(d_sd.zonename)) {
      ...
      if(!content.empty() && content.isPartOf(d_sd.zonename)) {
        lookup.emplace(content);
      }
    }
  }
...
void PacketHandler::emitNSEC3(DNSPacket& p, std::unique_ptr<DNSPacket>& r, const NSEC3PARAMRecordContent& ns3prc, const DNSName& name, const string& namehash, const string& nexthash, int mode) // NOLINT(readability-identifier-length)
{
  ...
  rr.dr.d_name = DNSName(toBase32Hex(namehash))+d_sd.zonename;
  rr.dr.d_ttl = d_sd.getNegativeTTL();
...
{
  DLOG(g_log<<"addNSEC3() mode="<<mode<<" auth="<<d_sd.zonename<<" target="<<target<<" wildcard="<<wildcard<<endl);
  ...
  if (!d_sd.db->doesDNSSEC() && !narrow) {
    ...
    g_log << Logger::Notice << "Backend for zone '" << d_sd.zonename << "' does not support DNSSEC operation, not adding NSEC3 hashes" << endl;
    return;
  }
  ...
  if (mode != 3) {
    ...
    if (((mode == 0 && ns3rc.d_flags) ||  mode == 1) && (hashed != before)) {
      ...
      bool doBreak = false;
      DNSZoneRecord rr;
      while( closest.chopOff() && (closest != d_sd.zonename))  { // stop at SOA
...
{
  DLOG(g_log<<"addNSEC() mode="<<mode<<" auth="<<d_sd.zonename<<" target="<<target<<" wildcard="<<wildcard<<endl);
  ...
  if (!d_sd.db->doesDNSSEC()) {
    ...
    // be set to true. While it would be nice to support such a zone
    // configuration, we don't. Log a warning and skip DNSSEC processing.
    g_log << Logger::Notice << "Backend for zone '" << d_sd.zonename << "' does not support DNSSEC operation, not adding NSEC records" << endl;
    return;
  }
...
bool PacketHandler::tryAuthSignal(DNSPacket& p, std::unique_ptr<DNSPacket>& r, DNSName &target) // NOLINT(readability-identifier-length)
{
  ...
  if(!DNSName(d_sd.zonename).hasLabels() || !pdns_iequals(DNSName(d_sd.zonename).getRawLabel(0), "_signal") || !d_dk.isSignalingZone(d_sd.zonename)) {
    return false;
  }
...
bool PacketHandler::opcodeQueryInner2(DNSPacket& pkt, queryState &state, bool retargeted)
{
  ...
  DLOG(g_log<<Logger::Error<<"We have authority, zone='"<<d_sd.zonename<<"', id="<<d_sd.domain_id<<", zonename="<<d_sd.zonename<<endl);

  if (!retargeted) {
    state.r->qdomainzone = d_sd.zonename;
  } else if (!d_doResolveAcrossZones && DNSName(state.r->qdomainzone) != DNSName(d_sd.zonename)) {
  ...
  if(pkt.qtype.getCode() == QType::SOA && pkt.qdomain == d_sd.zonename) {
  ...
  while(B.get(zrr)) {
    ...
    if (zrr.dr.d_type == QType::LUA && !isPresigned()) {
      ...
      if(rec->d_type == QType::CNAME || rec->d_type == pkt.qtype.getCode() || (pkt.qtype.getCode() == QType::ANY && rec->d_type != QType::RRSIG)) {
        state.noCache=true;
        try {
          auto recvec=luaSynth(rec->getCode(), state.target, zrr, d_sd.zonename, pkt, rec->d_type, s_LUA);
          if(!recvec.empty()) {
    ...
    if((zrr.dr.d_type == pkt.qtype.getCode() && !zrr.auth) || (zrr.dr.d_type == QType::NS && (!zrr.auth || !(zrr.dr.d_name == d_sd.zonename)))) {
      weHaveUnauth=true;
    }
  ...
  if(weRedirected) {
    ...
    //      g_log<<Logger::Error<<"Should not get here ("<<pkt.qdomain<<"|"<<pkt.qtype.toString()<<"): you have a trailing dot, this could be the problem (or run pdnsutil rectify-zone " <<d_sd.zonename<<")"<<endl;
    // } else {
         g_log<<Logger::Error<<"Should not get here ("<<pkt.qdomain<<"|"<<pkt.qtype.toString()<<"): please run pdnsutil rectify-zone "<<d_sd.zonename<<endl;
    // }
  }
  else {
