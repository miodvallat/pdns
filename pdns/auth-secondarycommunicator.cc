  ...
  try {
    ...
    while (xfr != fromXFR.end() || db != fromDB.end()) {
      ...
      if (create && remove) {
        ...
        if (di.zone != d.catalog && ci.d_coo == di.zone) {
    ...
    for (const auto& zone : clearCache) {
      ...
      purgeAuthCaches(DNSName(zone.first).toString() + "$");
    }
...
static bool catalogProcess(const DomainInfo& di, vector<DNSResourceRecord>& rrs, string logPrefix)
{
  ...
  for (auto& rr : rrs) {
    ...
    else if (rr.qname == g_versiondnsname + di.zone && rr.qtype == QType::TXT) {
      if (hasVersion) {
  ...
  try {
    ...
    for (const auto& d : deltas) {
      ...
      for (const auto& g : grouped) {
        ...
        di.backend->replaceRRSet(di.id, DNSName(g.first.first) + domain, QType(g.first.second), replacement);
      }
...
void CommunicatorClass::suck(const ZoneName& domain, const ComboAddress& remote, bool force) // NOLINT(readability-function-cognitive-complexity)
{
  ...
  try {
    ...
    purgeAuthCaches(DNSName(domain).toString() + "$");
