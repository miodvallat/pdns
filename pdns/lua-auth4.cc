...
/* DNSPacket */
  ...
  d_lw->registerMember<DNSPacket, DNSName>("qdomainzone", [](const DNSPacket &p) -> DNSName { return DNSName(p.qdomainzone); }, [](DNSPacket &p, const DNSName& name) { p.qdomainzone = ZoneName(name); });
