  ...
  void nextResult()
  {
    ...
    if (PQftype(d_res_set, 0) == 1790) { // REFCURSOR
      SLOG(g_log << Logger::Error << "Postgres query returned a REFCURSOR and we do not support those - see https://github.com/PowerDNS/pdns/pull/10259" << endl,
           d_slog->error(Logr::Error, "query returned a REFCURSOR which is not supported by PowerDNS", "more information", Logging::Loggable("https://github.com/PowerDNS/pdns/pull/10259")));
      PQclear(d_res_set);
      d_res_set = nullptr;
    }
    else {
