...
int PacketHandler::tryAutoPrimarySynchronous(const DNSPacket& p, const DNSName& tsigkeyname)
{
  ...
  if (!B.autoPrimaryBackend(remote.toString(), zonename, nsset, &nameserver, &account, &db)) {
    g_log << Logger::Error << "Unable to find backend willing to host " << zonename << " for potential autoprimary " << remote << ". Remote nameservers: " << endl;
...
std::unique_ptr<DNSPacket> PacketHandler::doQuestion(DNSPacket& p)
{
  ...
  retargeted:;
    ...
    if (retargetcount == 0) {
      r->qdomainzone = ZoneName(d_sd.qname);
    } else if (!d_doResolveAcrossZones && DNSName(r->qdomainzone) != d_sd.qname) {
    ...
    // this TRUMPS a cname!
    if(d_dnssec && p.qtype.getCode() == QType::NSEC && !d_dk.getNSEC3PARAM(ZoneName(d_sd.qname), nullptr)) {
    ...
    if(!doLua) {
      string val;
      d_dk.getFromMeta(ZoneName(d_sd.qname), "ENABLE-LUA-RECORDS", val);
      doLua = (val=="1");
    }
#endif
    ...
    while(B.get(rr)) {
#ifdef HAVE_LUA_RECORDS
      if (rr.dr.d_type == QType::LUA && !d_dk.isPresigned(ZoneName(d_sd.qname))) {
        if(!doLua)
          continue;
        auto rec=getRR<LUARecordContent>(rr.dr);
        if (!rec) {
      ...
      if (DP && rr.dr.d_type == QType::ALIAS && (p.qtype.getCode() == QType::A || p.qtype.getCode() == QType::AAAA || p.qtype.getCode() == QType::ANY) && !d_dk.isPresigned(ZoneName(d_sd.qname))) {
        if (!d_doExpandALIAS) {
    ...
    if(weRedirected) {
      ...
      bool presigned = d_dk.isPresigned(ZoneName(d_sd.qname));
