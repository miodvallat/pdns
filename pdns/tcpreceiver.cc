...
bool TCPNameserver::canDoAXFR(std::unique_ptr<DNSPacket>& q, bool isAXFR, std::unique_ptr<PacketHandler>& packetHandler)
{
  ...
  if(q->d_havetsig) { // if you have one, it must be good
    ...
    if (g_doGssTSIG && q->d_tsig_algo == TSIG_GSS) {
      ...
      packetHandler->getBackend()->getDomainMetadata(ZoneName(q->qdomain), "GSS-ALLOW-AXFR-PRINCIPAL", princs);
...
#endif
    if(!dk.TSIGGrantsAccess(ZoneName(q->qdomain), tsigkeyname)) {
  ...
  SOAData sd;
  if(packetHandler->getBackend()->getSOAUncached(ZoneName(q->qdomain),sd)) {
    // cerr<<"got backend and SOA"<<endl;
    vector<string> acl;
    packetHandler->getBackend()->getDomainMetadata(ZoneName(q->qdomain), "ALLOW-AXFR-FROM", acl);
  ...
  if(Communicator.justNotified(ZoneName(q->qdomain), q->getInnerRemote().toString())) { // we just notified this ip
...
int TCPNameserver::doAXFR(const DNSName &target, std::unique_ptr<DNSPacket>& q, int outsock)  // NOLINT(readability-function-cognitive-complexity)
{
  ...
  if(securedZone && !presignedZone) { // this is where the DNSKEYs, CDNSKEYs and CDSs go in
    ...
    dk.getPublishCDNSKEY(ZoneName(q->qdomain), publishCDNSKEY);
    dk.getPublishCDS(ZoneName(q->qdomain), publishCDS);
...
int TCPNameserver::doIXFR(std::unique_ptr<DNSPacket>& q, int outsock)
{
  ...
  bool serialPermitsIXFR;
  {
    ...
    // canDoAXFR does all the ACL checks, and has the if(disable-axfr) shortcut, call it first.
    if(!canDoAXFR(q, false, *packetHandler) || !(*packetHandler)->getBackend()->getSOAUncached(ZoneName(q->qdomain), sd)) {
      g_log<<Logger::Warning<<logPrefix<<"failed: not authoritative"<<endl;
      outpacket->setRcode(RCode::NotAuth);
      sendPacket(outpacket,outsock);
      return 0;
    ...
    DNSSECKeeper dk((*packetHandler)->getBackend());
    DNSSECKeeper::clearCaches(ZoneName(q->qdomain));
    bool narrow = false;
    securedZone = dk.isSecuredZone(ZoneName(q->qdomain));
    if(dk.getNSEC3PARAM(ZoneName(q->qdomain), nullptr, &narrow)) {
      if(narrow) {
