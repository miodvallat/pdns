  ...
  if(getSerial) {
    try {
      ...
      SLOG(g_log<<Logger::Error<<"Error retrieving serial for '"<<domain<<"': "<<ae.reason<<endl,
           d_slog->error(Logr::Error, "error retrieving serial", "zone", Logging::Loggable(domain), "error", Logging::Loggable(ae.reason)));
    }
  }
  ...
  for (const auto& row : d_result) { // id, name, type, master, last_check, catalog, content
    ...
    try {
      ...
      SLOG(g_log << Logger::Warning << __PRETTY_FUNCTION__ << " zone name '" << row[1] << "' is not a valid DNS name: " << e.what() << endl,
           d_slog->info(Logr::Warning, "zone name is not a valid DNS name", "zone", Logging::Loggable(row[1]), "error", Logging::Loggable(e.what())));
      continue;
    }
    catch (PDNSException& ae) {
      SLOG(g_log << Logger::Warning << __PRETTY_FUNCTION__ << " zone name '" << row[1] << "' is not a valid DNS name: " << ae.reason << endl,
           d_slog->info(Logr::Warning, "zone name is not a valid DNS name", "zone", Logging::Loggable(row[1]), "error", Logging::Loggable(ae.reason)));
      continue;
    }
    ...
    if (!row[5].empty()) {
      try {
        ...
        SLOG(g_log << Logger::Warning << __PRETTY_FUNCTION__ << " error while parsing SOA data for zone '" << di.zone << "': " << exp.what() << endl,
           d_slog->info(Logr::Warning, "error while parsing SOA data", "zone", Logging::Loggable(di.zone), "error", Logging::Loggable(exp.what())));
        continue;
      }
      catch (...) {
      ...
      try {
        ...
        SLOG(g_log << Logger::Warning << __PRETTY_FUNCTION__ << " could not convert last_check '" << row[4] << "' for zone '" << di.zone << "' into an integer: " << e.what() << endl,
           d_slog->info(Logr::Warning, "secondary 'last check' field is not a valid integer", "zone", Logging::Loggable(di.zone), "field", Logging::Loggable(row[4]), "error", Logging::Loggable(e.what())));
        continue;
      }
    ...
    try {
      ...
      SLOG(g_log << Logger::Warning << __PRETTY_FUNCTION__ << " could not convert id '" << row[0] << "' for zone '" << di.zone << "' into an integer: " << e.what() << endl,
           d_slog->info(Logr::Warning, "secondary 'domain id' field is not a valid integer", "zone", Logging::Loggable(di.zone), "field", Logging::Loggable(row[0]), "error", Logging::Loggable(e.what())));
      continue;
    }
    ...
    for (const auto& m : primaries) {
      try {
        ...
        SLOG(g_log << Logger::Warning << __PRETTY_FUNCTION__ << " could not parse primary address '" << m << "' for zone '" << di.zone << "': " << e.reason << endl,
           d_slog->info(Logr::Warning, "invalid primary address", "zone", Logging::Loggable(di.zone), "primary", Logging::Loggable(m), "error", Logging::Loggable(e.reason)));
      }
    }
  ...
  for (const auto& row : d_result) { // id, name, type, notified_serial, options, catalog, content
    ...
    try {
      ...
      SLOG(g_log << Logger::Warning << static_cast<const char *>(__PRETTY_FUNCTION__) << " could not convert id '" << row[0] << "' for zone '" << di.zone << "' into an integer: " << exp.what() << endl,
           d_slog->info(Logr::Warning, "primary 'domain id' field is not a valid integer", "zone", Logging::Loggable(di.zone), "field", Logging::Loggable(row[0]), "error", Logging::Loggable(exp.what())));
      continue;
    }
    ...
    try {
      ...
      SLOG(g_log << Logger::Warning << __PRETTY_FUNCTION__ << " zone name '" << row[5] << "' is not a valid DNS name: " << e.what() << endl,
           d_slog->info(Logr::Warning, "zone name is not a valid DNS name", "zone", Logging::Loggable(row[5]), "error", Logging::Loggable(e.what())));
      continue;
    }
    catch (PDNSException& ae) {
      SLOG(g_log << Logger::Warning << __PRETTY_FUNCTION__ << " zone name '" << row[5] << "' is not a valid DNS name: " << ae.reason << endl,
           d_slog->info(Logr::Warning, "zone name is not a valid DNS name", "zone", Logging::Loggable(row[5]), "error", Logging::Loggable(ae.reason)));
      continue;
    }
    ...
    try {
      ...
      SLOG(g_log << Logger::Warning << __PRETTY_FUNCTION__ << " catalog hash update failed'" << row[4] << "' for zone '" << di.zone << "' member of '" << di.catalog << "': " << e.what() << endl,
           d_slog->info(Logr::Warning, "catalog hash update failed", "zone", Logging::Loggable(di.zone), "catalog", Logging::Loggable(di.catalog), "error", Logging::Loggable(e.what())));
      continue;
    }
    ...
    try {
      ...
      SLOG(g_log << Logger::Warning << __PRETTY_FUNCTION__ << " could not convert notified_serial '" << row[4] << "' for zone '" << di.zone << "' into an integer: " << e.what() << endl,
           d_slog->info(Logr::Warning, "'notified serial' field is not a valid integer", "zone", Logging::Loggable(di.zone), "field", Logging::Loggable(row[4])));
      continue;
    }
    ...
    try {
      ...
      SLOG(g_log << Logger::Warning << __PRETTY_FUNCTION__ << " error while parsing SOA content '" << row[6] << "' for zone '" << di.zone << "': " << exp.what() << endl,
           d_slog->info(Logr::Warning, "error while parsing SOA data", "zone", Logging::Loggable(di.zone), "content", Logging::Loggable(row[6]), "error", Logging::Loggable(exp.what())));
      continue;
    }
    catch (...) {
  ...
  for (const auto& row : d_result) { // id, zone, options, [master]
    ...
    try {
      ...
      SLOG(g_log << Logger::Warning << __PRETTY_FUNCTION__ << " could not convert id '" << row[0] << "' for zone '" << ci.d_zone << "' into an integer: " << e.what() << endl,
           d_slog->info(Logr::Warning, "catalog 'domain id' field is not a valid integer", "zone", Logging::Loggable(ci.d_zone), "field", Logging::Loggable(row[0]), "error", Logging::Loggable(e.what())));
      members.clear();
      return false;
    }
    ...
    try {
      ...
      SLOG(g_log << Logger::Warning << __PRETTY_FUNCTION__ << " options '" << row[2] << "' for zone '" << ci.d_zone << "' is no valid JSON: " << e.what() << endl,
           d_slog->info(Logr::Warning, "catalog 'options' field is not valid JSON", "zone", Logging::Loggable(ci.d_zone), "field", Logging::Loggable(row[2]), "error", Logging::Loggable(e.what())));
      members.clear();
      return false;
    }
      ...
      for (const auto& m : primaries) {
        try {
          ...
          SLOG(g_log << Logger::Warning << __PRETTY_FUNCTION__ << " could not parse primary address '" << m << "' for zone '" << ci.d_zone << "': " << e.reason << endl,
               d_slog->info(Logr::Warning, "invalid primary address", "zone", Logging::Loggable(ci.d_zone), "primary", Logging::Loggable(m), "error", Logging::Loggable(e.reason)));
          members.clear();
          return false;
        }
      }
