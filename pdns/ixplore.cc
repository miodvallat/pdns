  ...
  try {
    ...
    if(command=="diff") {
      ...
      for(const auto& d : diff) {
        cout<<'-'<< (d.d_name+DNSName(zone)) <<" IN "<<DNSRecordContent::NumberToType(d.d_type)<<" "<<d.getContent()->getZoneRepresentation()<<endl;
      }
      diff.clear();
    ...
    for(;;) {
      ...
      for(const auto& delta : deltas) {
        ...
        for(const auto& rr : remove) {
          report<<'-'<< (rr.d_name+DNSName(zone)) <<" IN "<<DNSRecordContent::NumberToType(rr.d_type)<<" "<<rr.getContent()->getZoneRepresentation()<<endl;
