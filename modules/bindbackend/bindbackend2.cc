      ...
      if (qnames.count(shorter) == 0u) {
        if (!(maxent)) {
          SLOG(g_log << Logger::Error << "Zone '" << zoneName << "' has too many empty non terminals." << endl,
               d_slog->error(Logr::Error, "Zone has too many empty non terminals.", "zone", Logging::Loggable(zoneName)));
          return;
        }
  ...
  if (!getArg("config").empty()) {
    ...
    try {
      ...
      SLOG(g_log << Logger::Error << "Error parsing bind configuration: " << ae.reason << endl,
           d_slog->error(Logr::Error, "Error parsing bind configuration", "error", Logging::Loggable(ae.reason)));
      throw;
    }
    ...
    for (const auto& domain : domains) {
      if (!(domain.hadFileDirective)) {
        SLOG(g_log << Logger::Warning << d_logprefix << " Zone '" << domain.name << "' has no 'file' directive set in " << getArg("config") << endl,
             d_slog->error(Logr::Warning, "Zone has no 'file' directive set", "zone", Logging::Loggable(domain.name), "filename", Logging::Loggable(getArg("config"))));
        rejected++;
        continue;
      }
      ...
      if (filenameChanged || !bbd.d_loaded || !bbd.current()) {
        ...
        try {
          ...
          SLOG(g_log << Logger::Warning << d_logprefix << msg.str() << endl,
               d_slog->error(Logr::Error, "Error in zone file", "zone", Logging::Loggable(domain.name), "file", Logging::Loggable(domain.filename), "error", Logging::Loggable(ae.reason)));
          rejected++;
        }
        catch (std::system_error& ae) {
          ostringstream msg;
          bool missingNewSecondary = ae.code().value() == ENOENT && isNew && domain.type == "slave";
          ...
          SLOG(
            ...
            if (missingNewSecondary) {
              d_slog->error(Logr::Warning, "Secondary domain has not been AXFR'd yet", "domain", Logging::Loggable(domain.name), "file", Logging::Loggable(domain.filename));
            } else {
              d_slog->error(Logr::Warning, "Parse error", "domain", Logging::Loggable(domain.name), "file", Logging::Loggable(domain.filename), "error", Logging::Loggable(ae.what()));
            });
          rejected++;
        }
...
void Bind2Backend::queueReloadAndStore(domainid_t id)
{
  ...
  try {
    ...
    SLOG(g_log << Logger::Warning << "Error parsing '" << bbold.d_name << "' from file '" << bbold.main_filename() << "': " << ae.reason << endl,
         d_slog->error(Logr::Error, "Error reloading zone", "zone", Logging::Loggable(bbold.d_name), "file", Logging::Loggable(bbold.main_filename()), "error", Logging::Loggable(ae.reason)));
    ...
    SLOG(g_log << Logger::Warning << "Error parsing '" << bbold.d_name << "' from file '" << bbold.main_filename() << "': " << ae.what() << endl,
         d_slog->error(Logr::Error, "Error reloading zone", "zone", Logging::Loggable(bbold.d_name), "file", Logging::Loggable(bbold.main_filename()), "error", Logging::Loggable(ae.what())));
    bbold.d_status = msg.str();
    bbold.d_lastcheck = time(nullptr);
    bbold.d_wasRejectedLastReload = true;
    safePutBBDomainInfo(bbold);
...
bool Bind2Backend::autoPrimariesList(std::vector<AutoPrimary>& primaries)
{
  ...
  if (!c_if) {
    SLOG(g_log << Logger::Error << "Unable to open autoprimaries file for read: " << stringerror() << endl,
         d_slog->error(Logr::Error, "Unable to open autoprimaries file", "file", Logging::Loggable(filename), "error", Logging::Loggable(stringerror())));
    return false;
  }
...

  {
    ...
    if (!c_of) {
      ...
      SLOG(g_log << Logger::Error << "Unable to open autoprimary configfile for append: " << errorMessage << endl,
           d_slog->error(Logr::Error, "Unable to open autoprimary configuration file for append", "file", Logging::Loggable(configfile), "error", Logging::Loggable(errorMessage)));
